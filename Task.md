...
- R-跟进（**Phase Q**）：
  ...
  - [x] ⑬ **round-6 三项主动改进（用户方向：先 ASR 准确度 → 长尾稳定，本轮做 3 项微调，用户自测长会）+ 对抗式复审 4 补修**：三路并行只读调查定位后 subagent-driven 实现，每提交独立对抗式复审。**任务1 STT 识别准确度**（痛点"Imperial College London"被听成"Imperial Killedge"）：Speechmatics `additional_vocab` 自定义词表——静态英国留学域词表（`ASR_EXTRA_VOCAB` env，逗号/JSON 两态）+ `buildSessionVocab` 动态注入 roster 名+别名+面试官名，两条流都注入，`SPEECHMATICS_VOCAB` 总闸一键回滚，state 读失败降级静态词表；WebFetch 确认 RT API 支持 additional_vocab（≤1000 条/≤6 词，cmn_en 双语支持未文档化=已加自愈兜底，`16cf082`）。**任务3 Roster 花名册**（痛点自报"Kenny Tan"赢过花名册"Tina"）：reconcile 加 `rosterEntries` 结构化入参——1v1 单主名时 formal 自报名绑 roster 主名（preferred"call me X">roster>自报名，group≥2 主名保持自报名不误绑）+ alias 命中归一主名+sole-candidate 用主名判定（修 1v1 身份分裂）；hello 路径补 `extractNameAliases`（原只 POST /config 有）；desktop"Name (alias)"括号语法 `parseNameWithAliases`（`141d5e7`）。**任务2 双采集器合并**：`useAudioCapture` 308→80 行薄封装，`AudioService` 加 `startPreview/stopPreview`（不建 PCM processor、不碰 isCapturing、录音中 no-op 复用图杜绝双开麦克风）+ 删死 System/Mixed meter（`939b0f7`）。**对抗式复审（3 视角+逐发现核实，因会话额度中断 roster lens 与部分 verify 未跑完，由主 agent 读代码补判）确认 4 真问题并补修**：(1) **major｜STT invalid_config 无自愈**——若 cmn_en 拒 vocab（未验证能力）或用户名含怪字符→Error→readyReject→重连用同 vocab 再拒→死循环（sendQueue 无上限撞 DO 128MB）→ runtime 加 `vocabRejected`/`lastConnectSentVocab`，Error 分支带 vocab 时置位、下次重连丢 vocab 自愈（+3 测试）；(2) **major｜desktop initMic 无取消守卫**——preview 的 getUserMedia 被 macOS 授权框拖住时用户开会话/离开 Settings→续体 stop 掉当前 micStream（可能是直播会话麦克风）+泄漏新流 → 加 `micInitToken` generation 守卫，await 后 token 变则 stop 孤儿流并 return（+1 交错测试）；(3) minor｜词表 1000 cap 截断顺序倒挂（静态先入挤掉最高价值 roster 名）→ 会话名先 push 静态垫底（+1 测试）；(4) minor｜录音中会话 mic 坏→Settings 永久"Starting..."死角 → `sessionMicUnavailable` 诚实提示。顺手清静态词表 6 个 LM 已知常用词（`c627ed3`/`85a177d`）。**worker 807 / desktop 402 全绿，tsc/typecheck 清，vite build 通过，5 commit 零 co-author**。已部署 Worker Version **4322f431** + 重建 dist。**待用户长会自测**：①中英专有名词识别是否改善（尤其 roster 名+机构名）②30-60min 长会稳定性（keepalive/重连）③1v1 花名册名优先是否符合预期。**已知需 live 验证**：cmn_en 对 additional_vocab 的实际支持（若不支持，自愈会在首次连接丢 vocab 后正常转写，日志有 warn）。
