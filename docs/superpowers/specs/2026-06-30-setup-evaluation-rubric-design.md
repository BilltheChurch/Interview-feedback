...
- 自定义模板（localStorage）：`{ id, name, interview_type, dimensions: DimensionPresetItem[] }`，迁移现有 `ifb_rubric_templates`（旧 shape 无 key→载入时补生成 key）。
...
- 当前 `evaluation_dimensions` 已带 `weight`（恒 1.0），但 synthesis prompt 未必用它。改 `llm-synthesizer.ts` 的 system prompt（rule 4 / evaluation framework 处）：明确指示 LLM「按各维 `weight` 加权来形成 per-person 的 overall 评价与跨人排序——权重高的维度对总体结论影响更大」。
...
- 类型 pill 的英文命名（Academic/Technical/Behavioral/Group 直接用，或 General 等）。
- 旧 `ifb_rubric_templates` 迁移细节（无 key 旧模板的 key 生成）。
